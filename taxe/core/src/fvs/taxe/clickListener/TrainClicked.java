        ...
        if (train.getActor() != null && x != -1) {
            if (train.getActor().getPaused()) {
                ...
                if (stackedTrains.size() == 1) {
                    ...
                    DialogStationMultitrain dialog = new DialogStationMultitrain(stackedTrains, context.getSkin(), context);
        ...
        if (!displayingMessage) {
            ...
            if (Game.getInstance().getState() == GameState.NORMAL) {
                ...
                if (!train.isOwnedBy(currentPlayer))
                    ...
                    context.getTopBarController().displayMessage("Opponent's " + train.getName() + ". Speed: "
                            + train.getSpeed(), Color.RED);
                else if (train.getFinalDestination() == null)
                    //If the train is not placed then the name and speed of the train are displayed
                    context.getTopBarController().displayMessage("Your " + train.getName() + ". Speed: "
                            + train.getSpeed(), Color.BLACK);

                else
                    //If the train is the player's and placed then the name, speed and destination of the train are all displayed
                    context.getTopBarController().displayMessage("Your " + train.getName() + ". Speed: "
                            + train.getSpeed() + ". Destination: " + train.getFinalDestination().getName(), Color.BLACK);
            }
        }
    }
}
