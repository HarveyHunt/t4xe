...
public class GoalController {
    ...
    public void setColours(Color[] colours) {
        ...
        //then redraw the current player goals with the new colours
        showCurrentPlayerGoals();
    }
    ...
    public void showCurrentPlayerGoals() {
        ...
        for (Goal goal : currentPlayer.getGoals()) {
            //Necessary to check whether the goals are complete as completed
            //goals are not removed from the player's list of goals, without
            //this check complete goals would also be displayed.
            if (!goal.getComplete()) {
                ...
                //The goal buttons are scaled so that they do not overlap nodes
                //on the map, this was found to be necessary after changing the
                //way goals were displayed
                ...
                //Adds the listener to the button so that it will inform the
                //correct parts of the system
                GoalClickListener listener = new GoalClickListener(context, goal);
                button.setPosition(x, y);

                if (colours[index++] != null) {
                    //Sets the colour based on the values in the array. If the
                    //train is routing then these colours will match nodes on
                    //the map, otherwise they are all grey.
    ...
    private String playerHeader() {
        ...
        //It was necessary to apply a decimal format to the score as it is stored
        //a double which by default is "0.0", however that is not intuitive for
        //scoring as it should only be integer values.
